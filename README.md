...
## Diagnosztika és V038 validáció

A `Log másolása` automatikusan végigellenőrzi a fő kombinációkat. A V038-hoz mellékelt 2026-09-22-i log eredménye:
...
- A `release/sPg_salvage_eladasi_ar_V038.html` ugyanennek a verziózott standalone kiadása.
...
Current public data does not provide a verified CMATS base conversion ratio. Therefore V038 uses an explicit **1/6 = 16.67% estimated fallback base recovery**. It is shown as *unverified* in the UI. Dinyx and Pyrometric remain selectable, but without a method-specific CMATS audit they share the same fallback base recovery.

## V038 validation snapshot
...
- `release/sPg_salvage_eladasi_ar_V038.html` is the versioned standalone copy.
...
```text
.
├── index.html                              # GitHub Pages entry, V038
├── release/
│   └── sPg_salvage_eladasi_ar_V038.html  # standalone release copy
