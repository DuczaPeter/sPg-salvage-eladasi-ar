...
const requiredFiles=[
  ...
  'docs/RELEASE_STANDARD.md','docs/RELEASE-CONTRACT.md','docs/RELEASE-AUDIT-V4.1.md','docs/RELEASE-AUDIT-V4.2.md','tools/build-manifest.mjs',
  '.gitattributes','.gitignore','.github/workflows/static-check.yml','.github/ISSUE_TEMPLATE/bug_report.yml','.github/ISSUE_TEMPLATE/feature_request.yml','.github/pull_request_template.md',
...
(/^Standard version: V4\.2$/m.test(standard)?pass:fail)('RELEASE_STANDARD_VERSION','V4.2');
const standardSha=sha256(fs.readFileSync(path.join(root,'docs/RELEASE_STANDARD.md')));
(standardSha==='f3b1358844a9f04da5ea8bfd6fe87b3051bd052e753bd8451991b39f894972b2'?pass:fail)('RELEASE_STANDARD_CANONICAL_SHA256',standardSha);
...
const audit=fs.readFileSync(path.join(root,'docs/RELEASE-AUDIT-V4.2.md'),'utf8');
(/PACKAGE STATUS/.test(audit)&&/PUBLISHED RELEASE STATUS/.test(audit)&&/READY WITH LIMITATIONS/.test(audit)&&/Visual audit/.test(audit)?pass:fail)('RELEASE_AUDIT_PRESENT');

const version=JSON.parse(fs.readFileSync(path.join(root,'VERSION.json'),'utf8'));
(version.release==='V058-SellOnlyDeterministic'&&version.releaseStandardVersion==='V4.2'&&version.packageStatus==='READY WITH LIMITATIONS'&&version.repositoryPublication==='MANUAL BY USER'?pass:fail)('VERSION_METADATA',version);
...
// V4.2 section 69: every declared file must exist (dot-prefixed files included).
const inventory=JSON.parse(fs.readFileSync(path.join(root,'FILE-INVENTORY.json'),'utf8'));
const invBad=inventory.files.filter(f=>{const a=path.join(root,f.path);return !fs.existsSync(a)||fs.statSync(a).size!==f.bytes||sha256(fs.readFileSync(a))!==f.sha256}).map(f=>f.path);
(invBad.length===0?pass:fail)('INVENTORY_EXISTENCE_PARITY',invBad.length?invBad:`${inventory.files.length} files`);
const csBad=[];
for(const line of fs.readFileSync(path.join(root,'CHECKSUMS.sha256'),'utf8').split(/\r?\n/).filter(Boolean)){
  const m=line.match(/^([a-f0-9]{64})\s{2}(.+)$/); if(!m){csBad.push(`format:${line}`);continue}
  const a=path.join(root,m[2]); if(!fs.existsSync(a)||sha256(fs.readFileSync(a))!==m[1])csBad.push(m[2]);
}
(csBad.length===0?pass:fail)('CHECKSUMS_EXISTENCE_PARITY',csBad.length?csBad:'all listed files present and matching');
const manifest=JSON.parse(fs.readFileSync(path.join(root,'PACKAGE-MANIFEST.json'),'utf8'));
const manBad=(manifest.requiredArtifacts||[]).filter(p=>!exists(p));
(manifest.requiredArtifacts&&manBad.length===0?pass:fail)('MANIFEST_REQUIRED_ARTIFACTS_EXIST',manBad.length?manBad:'all present');
const dotMissing=['.gitattributes','.gitignore','.github/workflows/static-check.yml','.github/pull_request_template.md'].filter(p=>!exists(p));
(dotMissing.length===0?pass:fail)('DOT_PREFIXED_FILES_PRESENT',dotMissing.length?dotMissing:'present');

// V4.2 sections 70-72: deterministic line-ending policy; hash-bound evidence protected with -text.
const attrs=exists('.gitattributes')?fs.readFileSync(path.join(root,'.gitattributes'),'utf8').split(/\r?\n/):[];
const attrMissing=['* text=auto eol=lf','index.html -text','release/*.html -text','test-artifacts/** -text'].filter(l=>!attrs.includes(l));
(attrMissing.length===0?pass:fail)('LINE_ENDING_POLICY',attrMissing.length?attrMissing:'LF default; hash-bound files -text');
const crlf=inventory.files.filter(f=>{const b=fs.readFileSync(path.join(root,f.path));return !b.includes(0)&&b.includes(13)&&!f.path.startsWith('test-artifacts/')}).map(f=>f.path);
(crlf.length===0?pass:fail)('NO_CRLF_IN_NORMALIZABLE_TEXT',crlf.length?crlf:'none');

// V4.2 section 20: README.md is the full primary-language landing page, not a language selector.
const readme=fs.readFileSync(path.join(root,'README.md'),'utf8');
(/## Mire való\?/.test(readme)&&/## Használat/.test(readme)&&/README\.en\.md/.test(readme)?pass:fail)('README_FULL_PRIMARY_LANDING');

const failed=checks.filter(x=>x.status!=='PASS');
const out={
  app:'sPg Salvage Selling Price',
  version:'V058-SellOnlyDeterministic',
  releaseStandard:'V4.2',
  packageRevision:'V058-GitHub-Release-Standard-V4.2',
  generatedAt:new Date().toISOString(),
  staticGateStatus:failed.length?'FAIL':'PASS',
  repositoryPublication:'MANUAL BY USER',
  packageStatus:failed.length?'BLOCKED':'READY WITH LIMITATIONS',
  publishedReleaseStatus:'BLOCKED - post-publish fresh-clone verification pending',
  checks
};
console.log(JSON.stringify(out,null,2));
if(process.env.WRITE_GATE_SUMMARY==='1') fs.writeFileSync(path.join(root,'test-artifacts/V058/static-check-v4.2.json'),JSON.stringify(out,null,2)+'\n');
